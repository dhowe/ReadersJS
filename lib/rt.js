(function (window, undefined) {
  ...
  RiText.defaultFont = function (font, size) {
    ...
    var fontString = function(f, s) {
      if (/\.vlw$/.test(f)) {
        ...
        f = RiText.defaults.fontFamily;
      }
      RiText.defaults.fontFamily = f;
      RiText.defaults._font = createFont(f, s);
    }
    ...
    if (a.length === 0) { // 0-args
      ...
      if (!RiText.defaults._font) { // if we don't have a font, create it
        RiText.defaults._font = createFont(RiText.defaults.fontFamily, RiText.defaults.fontSize);
      }
    }
    else {
  ...
  RiText._getDefaultFont = function () {
    ...
    RiText.defaults._font = RiText.defaults._font ||
      createFont(RiText.defaults.fontFamily, RiText.defaults.fontSize);
  ...
  RiText.prototype = {
    ...
    _update: function () {
      ...
      if (!window.lastLog) window.lastLog = 1;

      if (this._faderAlive()) {
        //this._color = lerpCol(f.from, f.to, this._faderProgress);
        if (millis() - window.lastLog > 100) {
          var t = (floor(this._faderElapsed()/100)/10)+'s';
          console.log(t, this._color);
    ...
    colorTo: function (col, seconds, delay) {
      ...
      rt.fader.startTime = millis();
      rt.fader.duration = seconds * 1000;
      rt.fader.from = rt._color;
      rt.fader.to = col;

      // var rt = this;
      // setTimeout(function() {
      //   var startTime = millis();
      //   lerpCol(rt.fill(), toColorObj, );
      // }, delay * 1000);
    },
    ...
    font: function (font, size) { // TODO: cases for when arg1 is object & string
      ...
      if (a.length == 1) {
        ...
        if (is(font, S)) {
          ...
          this._font = createFont(font, RiText.defaults.fontSize);
          return this;
        }
