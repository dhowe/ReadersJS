...
(function (window, undefined) {
  ...
  RiText.defaultFont = function (font, size) {
    ...
    var fontObject = function(f, s) {
      if (isNode() && f.widths) { // use no-op
        RiText.renderer._textFont(f);
      }
    ...
    var fontString = function(f, s) {
      ...
      RiText.defaults._font = RiText.renderer._createFont(f, s);
    }
  ...
  RiText.prototype = {
    ...
    colorTo: function (colors, seconds, delay, callback, _type, _destroyOnComplete) {
      ...
      if (!is(colors, A)) {
        if (isNum(colors.r) && isNum(colors.g) && isNum(colors.b) && isNum(colors.a))
          colors = [ colors.r, colors.g, colors.b, colors.a ];
        else
          err('arg#1 to colorTo() must be an array');
      }
      ...
      var rt = this,
        id = setTimeout(function () {
          ...
          //this.fader && this.fader.stop();
          this.fader = new TextBehavior(rt, rt._color)
            .to({
              r: colors.r,
              g: colors.g,
              b: colors.b,
